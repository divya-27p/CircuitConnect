...
function ChatRoomMessage({ chatMessages, messageLoading }) {
  ...
  return (
    <RoomMessage>
      {chatId ? (
        messageLoading ? (
          <RoomEmptyMessage>Loading discussions...</RoomEmptyMessage>
        ) : chatMessages.length === 0 ? (
          <RoomEmptyMessage>Start discussing your circuit ideas ⚡</RoomEmptyMessage>
        ) : (
          renderedMessage
        )
      ) : (
        <RoomWelcomeMessage>Select an engineer to start collaborating</RoomWelcomeMessage>
      )}
    </RoomMessage>
  );
}
